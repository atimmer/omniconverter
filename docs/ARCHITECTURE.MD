...
- Some conversions are always possible on any input (e.g., base64 encode).
- Defined via `AlwaysPossibleModule` with a `heuristicScore` for ordering; resolved through `resolveAlwaysPossible` in `src/conversions/alwaysPossible.ts`.
- A separate “Always-on converters” section renders beneath the main results without altering the “no results” state.
- Route configs can provide `alwaysPossibleModuleId` so the matching converter is promoted to the top of this section.
...
export const CONVERSION_PAGES: ConversionPageConfig[] = [
  {
    ...
    description: "Convert RGB colors to HSL instantly.",
    biasModuleId: "color",
  ...
  },
  {
    ...
    description: "Quickly switch between metric and imperial weights.",
    biasModuleId: "mass",
    exampleInput: "70 kg",
  },
  {
    ...
    description:
      "Inspect JWT headers and payloads without validating signatures.",
    biasModuleId: "jwt",
...
- `detect`: accept standard base64 or base64url strings (length >= 8, mostly alphanumeric), normalize/pad, decode to UTF-8, and bail out if the result is not mostly printable text; try parsing JSON from the decoded text.
- `convert`: rows for `Decoded (UTF-8)` with a hint about the detected variant plus `Decoded JSON` when parsing succeeds.
