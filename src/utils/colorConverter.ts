...
const RGB_REGEX = /^rgba?\(\s*(?<r>-?\d+(?:\.\d+)?%?)\s*,\s*(?<g>-?\d+(?:\.\d+)?%?)\s*,\s*(?<b>-?\d+(?:\.\d+)?%?)\s*(?:,\s*(?<a>-?\d*(?:\.\d+)?%?)\s*)?\)$/i
...
const channelFromPercent = (value: number) => clamp((value / 100) * 255, 0, 255)
...
export function parseHexString(rawInput: string): RgbColor | null {
  ...
  const a = hasAlpha ? clamp(parseInt(hex.slice(6, 8), 16) / 255, 0, 1) : undefined
...
const formatRgbString = ({ r, g, b }: RgbColor) => `rgb(${Math.round(r)}, ${Math.round(g)}, ${Math.round(b)})`
const formatRgbaString = ({ r, g, b, a }: RgbColor) => `rgba(${Math.round(r)}, ${Math.round(g)}, ${Math.round(b)}, ${a ?? 1})`

export function convertColorString(input: string): ColorConversionResult | null {
